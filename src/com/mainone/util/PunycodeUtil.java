...
public class PunycodeUtil {
  ...
  public static void main(String[] args) throws Exception{
    if(args.length != 1) {
      System.out.println("usage:java PunycodeUtil gbkString");
      System.exit(1);
    }
    System.out.println(chinese2punycode(args[0]));
  }
   ...
   * */
  public static String chinese2punycode(String chstr) throws Exception {
    if (chstr == null || chstr.equals("")) {
      throw new Exception(input_string_null);
    }
    String punystr = "";
    try {
  ...
  public static String punycode2chinese(String punystr) throws Exception {
    if (punystr == null || punystr.equals("")) {
      throw new Exception(punystr_bad_input);
    }
    String chstr = "";
    try {
   ...
   */
  private static String punycode_cn(String s_input) throws Exception {
    if (s_input == null) {
      throw new Exception(input_string_null);
    }
    ...
    if (! ( (s_input.startsWith(u_big)) || (s_input.startsWith(u_small)))) {
      throw new Exception(uninum_bad_input);
    }
   ...
   * if fail to transfer , throw PunyExceptions
   * */
  private static String punycode_encode(String s_input) throws Exception {
    if (s_input == null) {
      throw new Exception(input_string_null);
    }

    /**如果字符串为全英文字符串,直接返回字符串本身*/
    ...
    for (j = 0; j < input_length; ++j) {
      if (input[j] < (0x80L)) {
        if (max_out - out < 2)
          throw new Exception(too_long_output);
    ...
    while (h < input_length) {
      ...
      if (m - n > (maxint - delta) / (h + 1))
        throw new Exception(Integer_overflow);
      delta += (m - n) * (h + 1);
   ...
   * if fail to transfer , throw PunyExceptions
   * */
  private static String cdntopuny(String cdns) throws Exception {
    if (cdns == null) {
      throw new Exception(input_string_null);
    }
    String rds = cdns;
  ...
  private static String punys_to_ocdn(String punys) throws Exception {
    if (punys == null) {
      throw new Exception(input_string_null);
    }
...
*/
   ...
   * */
  private static String punycode_decode(String s_input) throws Exception {
    if (s_input == null) {
      throw new Exception(input_string_null);
    }
